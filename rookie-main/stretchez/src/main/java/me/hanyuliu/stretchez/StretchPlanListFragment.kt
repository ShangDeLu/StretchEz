...
class StretchPlanListFragment : Fragment() {
    ...
    private val stretchPlanListViewModel: StretchPlanListViewModel by lazy {
        ViewModelProviders.of(this).get(StretchPlanListViewModel::class.java)
    }
...
//                    stretchPlanListViewModel.deleteStretchPlan(it)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        stretchPlanListViewModel.stretchPlanListLiveData.observe(
            viewLifecycleOwner,
            Observer { stretchPlans ->
                stretchPlans?.let {
                    Log.i(TAG, "GOT stretchPlans ${stretchPlans.size}")
...
//        val stretchPlans = stretchPlanListViewModel.stretchPlans
    ...
    private inner class StretchPlanHolder(view: View)
        ...
        fun bind (stretchPlan: StretchPlan) {
            ...
            // if a user deleted stretch plan last more than 30 days, then delete it completely
...
//            Toast.makeText(context, "${stretchPlan.title} pressed!", Toast.LENGTH_SHORT)
//                .show()
